...
class AlphaNeumericVirtualKeyboard(QtWidgets.QWidget):
    ...
    """
    def __init__(self, source, x_pos=400, y_pos=600, parent=None):
    ...
    def showAnimate(self, val):
        self.move(int(self.x_pos), int(2 * self.y_pos - self.y_pos * val / 25))
    ...
    def closeAnimate(self, val):
        self.move(int(self.x_pos), int(self.y_pos + self.y_pos * (val) / 25))
        if val == 25:
            self.hide()
